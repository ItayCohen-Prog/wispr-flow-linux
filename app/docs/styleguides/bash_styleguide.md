...
The shell-script conventions for the Wispr Flow for Linux build pipeline
(`build.sh`, `scripts/*.sh`). The body below is forked from Dave Eddy's YSAP
guide; it is the convention the project's scripts already follow (tabs, `[[ ]]`,
`local` in functions, `$(...)`, no `eval`). `shellcheck` runs in CI
(`.github/workflows/shellcheck.yml`) and is the enforcement mechanism.

Project-specific notes:

- **Env var prefix is `WISPR_*`** for user-facing overrides (e.g.
  `WISPR_NATIVE_FLOWBAR`, `WISPR_DISABLE_GPU`), not `CLAUDE_*`.
...
- The launcher library (`scripts/launcher-common.sh`) and diagnostics
  (`scripts/doctor.sh`) are sourced, not exec'd — keep functions `local`-clean.
