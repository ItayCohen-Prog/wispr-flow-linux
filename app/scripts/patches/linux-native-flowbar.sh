...
bridge = (
    ...
    'const connect=()=>{const s=net.createConnection({path});let buf="",dropped=false;'
    ...
    'const drop=()=>{if(dropped)return;dropped=true;s.destroy();if(st.sock===s){st.sock=null;st.connected=false}'
