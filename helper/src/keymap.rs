...
// ---------------------------------------------------------------------------
// evdev / uinput mapping.
//
// The Wayland backend injects via /dev/uinput, which speaks Linux kernel
// `KEY_*` codes from <linux/input-event-codes.h>. These are physical-key codes
// on a US layout, so the same VK the app sends maps to the US-positioned evdev
// key. (Non-US layouts would shift the produced character.)
// ---------------------------------------------------------------------------
